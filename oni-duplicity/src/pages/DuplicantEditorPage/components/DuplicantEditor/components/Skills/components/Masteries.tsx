...
const Masteries: React.FC<Props> = ({ gameObjectId }) => {
  ...
  // Usa apenas as skills que já estão no MasteryBySkillID (skills reais do duplicante)
  // Isso garante que bionics mostrem apenas suas skills válidas, não todas as skills de humanos
  const availableSkills = MasteryBySkillID.map(m => m[0]).sort();
